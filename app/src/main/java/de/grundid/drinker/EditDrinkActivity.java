...
/*    FIXME        if (menuDrink.getVolume() != null)
					...
					public void onCompleted(Exception e, Response<InputStream> response) {
						if (e == null) {
							try {
								...
								//TODO
								categorySpinner.setAdapter(new StringListAdapter(categories.getCategories(),
										LayoutInflater.from(EditDrinkActivity.this),
	...
	private synchronized void saveDrink() {
		if (!saveInProcess) {
			...
			volumePrice.setVolume(DrinkModelHelper.parseDrinkVolume(drinkVolume.getText()));
			volumePrice.setPrice(DrinkModelHelper.parseDrinkPrice(drinkPrice.getText()));
			drinkModel.addVolumePrice(volumePrice);
